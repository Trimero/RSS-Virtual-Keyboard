...
function hardKeys() {
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Shift' && isRu === false) {
      isShiftPressed = true;
      pressedShift();
      const buttonShift = document.querySelector(
    ...
    if (e.key === 'CapsLock') {
      ...
      if (isCapsLockPressed) {
        for (let i = 0; i < letterKeys.length; i += 1) {
          if (!Number.isNaN(parseInt(keys[i], 10)) || ignoregKeys.includes(keys[i])) {
            letterKeys[i].textContent = keys[i];
          } else {
            letterKeys[i].textContent = keysUpperCaseEng[i];
            const buttonCaps = document.querySelector(
...
keys.forEach((key) => {
  ...
  button.addEventListener('click', () => {
    ...
    if (key === 'Backspace') {
      ...
      inputTextarea.value = inputTextarea.value.slice(0, startPos - 1)
        + inputTextarea.value.slice(inputTextarea.selectionStart);
      inputTextarea.setSelectionRange(startPos - 1, startPos - 1);
    }

    if (key === 'Del') {
      const startPos = inputTextarea.selectionStart;
      inputTextarea.value = inputTextarea.value.slice(0, startPos)
        + inputTextarea.value.slice(inputTextarea.selectionStart + 1);
    ...
    if (key === 'Enter') {
      const startPos = inputTextarea.selectionStart;
      inputTextarea.value = `${inputTextarea.value.substring(
        0,
        startPos,
    ...
    if (key === 'CapsLock') {
      ...
      if (isCapsLockPressed) {
        ...
        for (let i = 0; i < letterKeys.length; i += 1) {
          ...
          buttonCaps.classList.remove('keyboard-button-active');
        }
      }
    }
    if (key === 'Shift') {
